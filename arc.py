...
SYSTEM_PROMPTS = {
    'claude': (
        ...
        "EMPIRICAL VERIFICATION: When attributing a failure or limitation to an "
        "external system (OS behavior, library bug, hardware constraint, protocol "
        "limitation), you MUST propose a minimal test script (<30 lines) that "
        "verifies the claim in isolation. Do not build architectural recommendations "
        "on unverified assumptions about how external systems behave."
    ),
    'gpt': (
        ...
        "EMPIRICAL VERIFICATION: If the Builder claims a limitation of an external "
        "system (OS, library, protocol) without providing a test script that proves "
        "it, flag this as an unverified assumption. Propose an alternative test that "
        "could disprove the claim. Do not accept 'it can't be done' without evidence. "
    ...
    'gemini': (
        ...
        "- Unverified external assumptions: any claim about OS behavior, "
        "library limitations, hardware constraints, or protocol behavior "
        "that was accepted as fact without a test script proving it. "
        "AI models are excellent at reasoning from premises but terrible at "
        "questioning whether the premises are true. Flag every claim about "
        "how an external system behaves that lacks empirical verification.\n"
